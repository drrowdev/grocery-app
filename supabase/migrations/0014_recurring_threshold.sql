-- Phase: make recurrence achievable for real grocery patterns.
--
-- The previous gate (cnt >= 3 AND confidence >= 0.5) was effectively
-- unreachable: confidence = (cnt/6) * (1 - gap_variability), and real
-- grocery buying is bursty, so the variability term drove confidence to
-- near-zero even for clear staples (e.g. an item bought 8 times in a month
-- scored 0.04). Result: nothing was ever flagged is_recurring and the AI
-- suggestion card was permanently empty.
--
-- New design:
--   * is_recurring is a COUNT signal — bought >= 3 times = a staple.
--   * confidence is now count-dominant (full weight by the 4th purchase)
--     with the gap-variability penalty capped at 50%, so it stays a useful
--     ranking/sort signal without ever gating an item out entirely.
-- The next_predicted_date + horizon window still controls WHEN an item
-- actually surfaces, so we don't spam every staple at once.
...
begin
  ...
  -- Confidence: count-dominant (full weight by the 4th buy) with the gap-
  -- variability penalty capped at 50% so a bursty staple still scores
  -- meaningfully. Used for ranking/sorting suggestions, NOT as a gate.
  conf := least(1.0, cnt::numeric / 4.0)
          * (1.0 - 0.5 * least(1.0, coalesce(std_days / nullif(avg_days, 0), 1)));

  -- Recurring = a real staple: bought at least 3 times. When (and whether)
  -- it surfaces is then governed by next_predicted_date vs the suggestion
  -- horizon and whether it's already on a list.
  is_rec := (cnt >= 3);
...
-- Recompute every existing profile so the new recurrence flag + confidence
-- take effect immediately for already-recorded purchase history.
do $$
declare
  it uuid;
begin
  for it in select distinct item_id from public.purchases loop
